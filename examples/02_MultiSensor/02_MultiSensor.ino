...
// Contoh fungsi simulasi pembacaan sensor
...
void setup() {
  ...
  // 1. Tambah sensor suhu (dibaca setiap 5 detik)
  iot.addSensor("temp_dht", "Suhu Ruangan", "°C", readTemperature, 5000);

  // 2. Tambah sensor kelembapan (dibaca setiap 5 detik)
  iot.addSensor("hum_dht", "Kelembapan Udara", "%", readHumidity, 5000);

  // 3. Tambah sensor tanah analog di pin A0 (dibaca setiap 10 detik)
  iot.addSensor("soil_moist", "Kelembapan Tanah", "ADC", readSoilAnalog, 10000);

  // 4. Tambah aktuator relay jika ingin otomatisasi pompa
